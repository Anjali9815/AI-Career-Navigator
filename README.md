...
Current corpus: 25 LinkedIn profile exports.
...
| Ordering | **Plain Python** (`timeline.py`) | career steps are sorted by date in code, not generated by the model |
| API | **FastAPI** + Uvicorn | auto generated docs at `/docs`, lifespan for startup loading |
| Frontend | **React 18** + **Vite** | single page, no UI framework, plain CSS with design tokens |
| Logging | **stdlib logging** | console plus `logs/app.log`, one shared configuration |
...
The response schema deliberately has **no `path` field**. An earlier version asked
the model to list each person's career steps, and it returned them out of
chronological order and occasionally dropped a degree entirely.

The dates are already in `profiles.json`, so ordering is a sorting problem rather
than a language problem. `timeline.py` merges a person's education and experience
into one list, parses the four date formats that appear in the data
(`"August 2022"`, `"2015"`, `"2023-12"`, `"Present"`), and sorts oldest first.
Entries with no start date sort last, because an unknown date is not the same as
an early one.

The generated answer contributes the name; the timeline is attached to it after
generation by lookup. What the model cannot generate, it cannot get wrong.
...
A single page React app built with Vite. No UI framework: plain CSS with design
tokens on `:root`, so the whole palette changes from one block.

A student types their background and goal, and the page renders the structured
response as cards: the direction that fits them, one card per matched alumni
with a vertical timeline of that person's career, skill tags, and a numbered
list of next steps.

```
frontend/src/
├── main.jsx              mounts App, imports the stylesheet
├── App.jsx               state and layout only
├── api.js                every backend call
├── constants.js          example queries
├── utils.js              pure helpers
├── styles/index.css      tokens and all component classes
└── components/           Header, SearchBox, Examples, Notice,
                          ResultView, DirectionPanel, MatchList,
                          MatchCard, Timeline, SkillTags, NextSteps
```

Three conventions: `App.jsx` owns all state and components only render; every
network call goes through `api.js`, so pointing at a deployed backend is one
constant; and education steps render with hollow markers while jobs render
filled, so a degree reads differently from a role at a glance.
...
**Current: 0 errors across 25 profiles.**
...
**Current: 0.80 correct_retrieval** across 25 profiles.
**Latency: P50 1.4s, P99 1.8s** (dominated by the cross encoder).

Worth noting what holding steady means here: the score was 0.79 at 21 profiles
and 0.80 at 25. Adding profiles makes retrieval harder, since there is more to
be confused by, so an unchanged score after a re extraction and four new people
is the result the benchmark exists to confirm. Each run writes a timestamped CSV
to `evals/results/`, so two runs can be diffed without opening LangSmith.
...
**Let code do what code does well.** The model chooses who matches; it does not
sort dates, and it does not decide response length. Both were moved out of the
prompt and into the schema and into `timeline.py` after the model got them
wrong. Every field removed from what the model generates is a field it can no
longer hallucinate.
...
│       ├── timeline.py         date parsing and career step ordering
...
│   └── results/                timestamped CSVs per run
├── data/
│   ├── docs/                   source PDFs
...
It opens on port 3000 and expects the API on port 8000.
...
**Corpus size.** 25 profiles, all drawn from one university network. The retrieval techniques are scale independent, but the benchmark numbers are not: 20 test cases over 21 documents is a small sample, and the threshold gap is narrow enough that it will need retuning as the corpus grows.
...
- Extend the retrieval benchmark to cover the newly added profiles
...
- Surface `linkedin_url`, already extracted, as a Connect button on each match card
- Resolve the groundedness judge: hand label a small set, then tune the judge prompt against it
